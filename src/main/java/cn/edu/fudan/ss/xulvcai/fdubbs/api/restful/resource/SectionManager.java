...
public class SectionManager {
	...
	public Response getSectionDetail(@CookieParam("auth_code") String authCode, @PathParam("section_id") String sectionId) {
		...
		try {
			...
			return Response.status(RESTErrorStatus.REST_SERVER_PAMAMETER_ERROR_STATUS).build();
		} catch (Exception e) {
			logger.error("Exception occurs in getAllSectionsMetaData!", e);
			return Response.status(RESTErrorStatus.REST_SERVER_INTERNAL_ERROR_STATUS).build();
		}
		...
		Response response;
		if(section == null) {
			response = Response.status(RESTErrorStatus.REST_SERVER_INTERNAL_ERROR_STATUS).build();
		} else {
